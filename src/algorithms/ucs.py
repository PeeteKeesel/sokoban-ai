...
from utils import get_search_algorithm_results, \
    MESSAGE_SOLUTION_FOUND, MESSAGE_TIME_LIMIT_REACHED, ALGORITHM_NAME_UCS, \
    METRICS_SCELETON
...
def uniform_cost_search(env, time_limit: int, metrics: dict=None, print_steps: bool=None):
    ...
    while True:         # while the queue is not empty
        ...
        if not env_queue:
            raise Exception(f"{ALGORITHM_NAME_UCS}(): Solution NOT FOUND!"
                            "Empty environment queue.")

        node_env_cost, node_env = env_queue.pop()   # get the environment with the lowest cost

        if current_time >= time_limit:
            return get_search_algorithm_results(ALGORITHM_NAME_UCS,
                                                node_env, metrics,
